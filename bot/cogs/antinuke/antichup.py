...
class AntiChannelUpdate(commands.Cog):
    ...
    async def on_guild_channel_update(self, before, after):
        ...
        async with aiosqlite.connect("db/anti.db") as db:
            ...
            async with db.execute(
                "SELECT chup FROM whitelisted_users WHERE guild_id = ? AND user_id = ?",
                (guild.id, executor.id),
            ) as cursor:
    ...
    async def revert_channel_update_and_ban(self, before, after, executor, retries=3):
        while retries > 0:
            try:
                await after.edit(
                    ...
                    user_limit=(
                        before.user_limit if hasattr(before, "user_limit") else None
                    ),
                    reason="Modification de salon | Utilisateur non autorisé",
                )
                break
        ...
        while retries > 0:
            try:
                await before.guild.ban(
                    executor, reason="Modification de salon | Utilisateur non autorisé"
                )
                return
            except discord.Forbidden:
                return
