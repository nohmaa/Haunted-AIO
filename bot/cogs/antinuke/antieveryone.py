...
class AntiEveryone(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        async with aiosqlite.connect("db/anti.db") as db:
            ...
            try:
                ...
                print(
                    f"An unexpected error occurred while handling {message.author.id}: {e}"
                )
    ...
    async def timeout_user(self, user):
        ...
        while retries > 0:
            try:
                await user.edit(
                    ...
                    reason="Mention Everyone/Here | Utilisateur non autorisé",
                )
                return
                ...
                if e.status == 429:
                    ...
                    if retry_after:
                        ...
                        print(
                            f"Rate limit encountered while timing out. Retrying after {retry_after} seconds."
                        )
                ...
                print(
                    f"Rate limit encountered while timing out: {e}. Retrying in {e.retry_after} seconds."
                )
        ...
        print(
            f"Failed to timeout {user.id} after multiple attempts due to rate limits."
        )
            ...
            try:
                ...
                if e.status == 429:
                    ...
                    if retry_after:
                        retry_after = float(retry_after)
                        print(
                            f"Rate limit encountered while deleting messages. Retrying after {retry_after} seconds."
                        )
                        await asyncio.sleep(retry_after)
                        retries -= 1
                else:
                ...
                print(
                    f"Rate limit encountered while deleting messages: {e}. Retrying in {e.retry_after} seconds."
                )
                await asyncio.sleep(e.retry_after)
                retries -= 1
            except Exception as e:
