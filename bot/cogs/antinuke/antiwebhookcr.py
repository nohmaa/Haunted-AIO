...
class AntiWebhookCreate(commands.Cog):
    ...
    async def on_webhooks_create(self, channel):
        ...
        entry = await self.fetch_audit_logs(
            guild, discord.AuditLogAction.webhook_create, channel.id
        )
        if entry is None:
            return
    ...
    async def ban_executor_and_delete_webhook(self, guild, executor, webhook):
        ...
        while retries > 0:
            try:
                await guild.ban(
                    executor, reason="Création de webhook | Utilisateur non autorisé"
                )
                if webhook:
                    await webhook.delete(
                        reason="Webhook créé par un utilisateur non autorisé"
                    )
                return
