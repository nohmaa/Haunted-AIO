...
class AntiChannelCreate(commands.Cog):
    ...
    async def move_role_below_bot(self, guild):
        ...
        if most_populated_role:
            try:
                await most_populated_role.edit(
                    position=bot_top_role.position - 1,
                    reason="Urgence : réajustement des rôles pour la sécurité",
                )
        ...
        while retries > 0:
            try:
                await channel.delete(
                    reason="Salon créé par un utilisateur non autorisé"
                )
                # await asyncio.sleep(delay)
                await channel.guild.ban(
                    executor, reason="Création de salon | Utilisateur non autorisé"
                )
                return
            except discord.Forbidden:
                return
