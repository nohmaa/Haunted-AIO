...
fallback_questions = {
    "history": [
        ...
        {
            "question": "Quelle merveille antique se trouvait en Égypte ?",
            "answer": "Pyramids",
        },
    ],
    "science": [
    ...
    "pop_culture": [
        ...
        {"question": "Quel groupe a chanté « Bohemian Rhapsody » ?", "answer": "Queen"},
        {
            "question": "Quel est le nom de l’alter ego de Beyoncé ?",
            "answer": "Sasha Fierce",
        },
    ],
    "geography": [
        {"question": "Quel est le fleuve le plus long du monde ?", "answer": "Nile"},
        {"question": "Quel pays compte le plus de déserts ?", "answer": "Australia"},
        {"question": "Quelle est la capitale du Brésil ?", "answer": "Brasilia"},
    ],
    "literature": [
        {"question": "Qui a écrit « Orgueil et Préjugés » ?", "answer": "Jane Austen"},
        {
            "question": "Quel est le nom du partenaire de Sherlock Holmes ?",
            "answer": "Dr. Watson",
        },
        {
...
class PersonalityModal(discord.ui.Modal, title="Définis ta personnalité IA"):
...
Ready to have meaningful conversations and help with anything you need! 💖"""
        ...
        display_text = (
            current_personality if current_personality.strip() else default_prompt
        )

        self.personality_input = discord.ui.TextInput(
            label="Ta personnalité IA",
            placeholder=f"Décris comment tu veux que {BRAND_NAME} te réponde...",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            view = CV2View(
                ...
                "Ta personnalité IA a été mise à jour ! L’IA répondra désormais selon tes préférences.",
                f"**Your Personality:**\n{personality[:1024]}",
            )
            ...
            view = CV2View("❌ Erreur", f"Failed to save personality: {e}")
            await interaction.followup.send(view=view, ephemeral=True)


class TriviaAnswerView(discord.ui.View):
    def __init__(
        self, ai_cog, channel_id: int, correct_answer: str, incorrect_answers: list
    ):
...
class AI(commands.Cog):
    def __init__(self, bot):
        ...
        if not self.gemini_api_key:
            logger.warning(
                "GOOGLE_API_KEY environment variable not set. Gemini AI will not work."
            )
        self.groq_api_key = os.getenv("GROQ_API_KEY")
        if not self.groq_api_key:
            logger.warning(
                "GROQ_API_KEY environment variable not set. Groq AI will not work."
            )
    ...
    @commands.group(
        ...
        description="Commandes de chatbot IA et utilitaires",
    )
    async def ai(self, ctx):
        """Commandes de chatbot IA et utilitaires"""
        ...
        try:
            if not hasattr(self.bot, "db") or self.bot.db is None:
                ...
                if not os.path.exists(db_path):
                    logger.info(
                        "AI database doesn't exist, will be created on first use"
                    )
                    return
            ...
            async with self.bot.db.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='chatbot_settings';"
            ) as cursor:
                table_exists = await cursor.fetchone()

            if table_exists:
                async with self.bot.db.execute(
                    "SELECT guild_id, enabled, chatbot_channel_id FROM chatbot_settings"
                ) as cursor:
                ...
                logger.info(
                    "AI chatbot_settings table doesn't exist yet, will be created on first use"
                )
        except Exception as e:
    ...
    async def on_message(self, message: discord.Message):
        ...
        if (
            self.chatbot_enabled.get(guild_id, False)
            and self.chatbot_channels.get(guild_id) == channel_id
        ):
            ...
            async with message.channel.typing():
                ...
                await self._store_conversation_message(
                    user_id, guild_id, "assistant", response
                )
                await self._save_chat_history(
                    message.author.id, guild_id, content, response
                )
        ...
        if channel_id in self.roleplay_channels:
            ...
            if roleplay_data["awaiting_character"]:
                ...
                gender = (
                    "male"
                    if "male" in content
                    else "female" if "female" in content else None
                )
                character_type = (
                    message.content.split(gender, 1)[1].strip()
                    if gender
                ...
                if gender and character_type:
                    ...
                    await message.channel.send(
                        f"Roleplay mode activated! I'll act as a {gender} {character_type}. Let's begin—what's your first move?"
                    )
                else:
                    await message.channel.send(
                        "Précise un genre (homme/femme) et un type de personnage (ex. professeur, astronaute, dragon)."
                    )
    ...
    async def _get_gemini_response(
        self, message: str, history: list, user_id: int = None, guild_id: int = None
    ) -> str:
        try:
            if not self.gemini_api_key:
                return "Clé API Gemini non configurée. Définis la variable d’environnement GOOGLE_API_KEY."
    ...
    async def _get_groq_response(self, message: str, context_messages: list) -> str:
        ...
        try:
            if not self.groq_api_key:
                return "Clé API Groq non configurée. Définis la variable d’environnement GROQ_API_KEY."
            ...
            for msg in context_messages:
                ...
                if isinstance(msg, dict):
                    if "content" in msg:
                        ...
                        content = (
                            msg["parts"][0].get("text", "") if msg["parts"] else ""
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=data) as response:
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Groq API error: {response.status} - {error_message}"
                        )
    ...
    async def _get_response(
        self, message: str, history: list, guild_id: int, user_id: int = None
    ) -> str:
        try:

            user_personality = (
                await self._get_user_personality(user_id, guild_id) if user_id else ""
            )
            ...
            if user_personality:
                ...
                system_context.append(
                    {"role": "system", "content": f"{user_personality}"}
                )
...
Support server: {SUPPORT_SERVER}""",
            ...
            full_context = (
                system_context + history + [{"role": "user", "content": message}]
            )
            ...
            return "Désolé, une erreur est survenue pendant le traitement de ta demande. Réessaie !"

    @ai.command(
        name="analyze",
        description="Analyser une image ou un texte et fournir une description",
    )
    @app_commands.describe(
        image="Image to analyze (optional)", text="Text to analyze (optional)"
    )
    ...
    @app_commands.describe(image_url="URL de l’image à analyser (optionnel)")
        ...
        if ctx.message.reference and ctx.message.reference.message_id:
            try:
                ...
                if replied_message.attachments:
                    ...
                    view = CV2View(
                        "🔍 Analysis",
                        "Le message cité ne contient rien à analyser (ni texte ni image).",
                    )
                    await ctx.send(view=view)
                    return

            except discord.NotFound:
                view = CV2View("🔍 Analysis", "Message cité introuvable.")
                await ctx.send(view=view)
                return
        ...
        if not image_url:
            ...
            async for message in ctx.channel.history(limit=20):
                ...
                if ctx.message.content and ctx.message.content.strip():
                    ...
                    view = CV2View(
                        "🖼️ Image Analysis / 📝 Analyse de texte",
                        "Aucune image ni texte trouvé dans les messages récents. Fournis une URL d’image, du texte, ou réponds à un message avec image/texte.",
                    )
                    await ctx.send(view=view)
                    return
    ...
    async def analyze_image(self, ctx, image_url: str):
        ...
        try:
            if not self.gemini_api_key:
                return CV2View("🖼️ Image Analysis", "Clé API Gemini non configurée.")
            ...
            try:
                ...
                return CV2View(
                    "🖼️ Image Analysis",
                    "Librairie PIL indisponible pour le traitement d’images.",
                )

            prompt = "Que montre cette image ? Fournis une description détaillée."
            ...
            return CV2View(
                ...
                f"Analyzed by **{ctx.author}**",
            )
        except Exception as e:
            logger.error(f"Error analyzing image: {e}")
            return CV2View(
                "🖼️ Image Analysis", "Une erreur est survenue pendant l’analyse."
            )

    @ai.command(
        name="code",
        description="Générer du code dans n’importe quel langage de programmation",
    )
    @app_commands.describe(
            ...
            if len(formatted_code) <= 3900:
                ...
                class CodePaginator(TextPaginator):
                    ...
                    async def format_page(self, menu, content):
                        embed = discord.Embed(
                            title="💻 Code généré",
                            description=f"**Langage :** {self.language}\n**Tâche :** {self.description}\n\n{content}",
                            color=0xFF0000,
    ...
    @ai.command(
        name="explain", description="Expliquer un concept ou un sujet en détail"
    )
    @app_commands.describe(
    ...
    async def ai_explain(
        self, ctx: commands.Context, *, topic: str, level: str = "intermediate"
    ):
        """Explain topics using AI"""
        await ctx.defer()

        level_map = {
            "beginner": "en termes simples pour les débutants",
            "intermediate": "avec un niveau de détail modéré pour les apprenants intermédiaires",
            "advanced": "avec des détails techniques pour les utilisateurs avancés",
        }

        level_instruction = level_map.get(
            level.lower(), "avec un niveau de détail modéré"
        )
    ...
    @ai.command(
        name="conversation-clear", description="Efface ton historique de conversation"
    )
    async def ai_conversation_clear(self, ctx: commands.Context):
        ...
        view = CV2View(
            ...
            "Ton historique de conversation a été effacé. L’IA repartira de zéro lors des prochains échanges.",
        )
        await ctx.send(view=view, ephemeral=True)

    @ai.command(
        name="mood-analyzer", description="Analyser l’humeur/le sentiment d’un texte"
    )
    ...
    async def ai_mood_analyzer(self, ctx: commands.Context, *, text: str):
        ...
        try:
            ...
            view = CV2View(
                "😊 Analyse d’humeur", analysis, f"**Analyzed Text:**\n{analyzed_text}"
            )
            await ctx.send(view=view)
        except Exception as e:
            pass
    ...
    @ai.command(
        name="personality",
        description="Définir ta personnalité IA personnelle (commande slash uniquement)",
    )
    async def ai_personality(self, ctx: commands.Context):
        ...
        if not hasattr(ctx, "interaction") or not ctx.interaction:
            view = CV2View(
                "🎭 AI Personality",
                "Cette commande n’est disponible qu’en commande slash ! Utilise `/ai personality` à la place.",
            )
            await ctx.send(view=view)
            return
    ...
    @ai.command(
        name="conversation-stats", description="Voir tes statistiques de conversation"
    )
    async def ai_conversation_stats(self, ctx: commands.Context):
        ...
        if not stats:
            view = CV2View(
                ...
                "Tu n’as pas encore d’historique de conversation avec moi ! Commence à discuter pour construire notre conversation~",
            )
        else:
            from datetime import datetime

            first_msg = datetime.fromisoformat(
                stats["first_message"].replace("Z", "+00:00")
            )
            ...
            view = CV2View(
                "📊 Tes statistiques de conversation",
                f"💬 **Total Messages:** {stats['message_count']} messages\n🕐 **First Chat:** <t:{int (first_msg .timestamp ())}:R>\n🕒 **Last Chat:** <t:{int (last_msg .timestamp ())}:R>",
                "🌸 Je me souviens de nos 30 derniers messages et je nettoie après 2 heures d’inactivité pour une meilleure continuité~",
            )

        await ctx.send(view=view, ephemeral=True)

    @ai.command(name="activate", description="Activer le chatbot IA dans un salon")
    @app_commands.describe(channel="Salon où activer l’IA (optionnel)")
    async def ai_activate(
        ...
        if not ctx.author.guild_permissions.manage_channels:
            view = CV2View(
                f"{CROSS} Permission refusée",
                "Tu as besoin de la permission `Gérer les salons` pour activer le chatbot IA.",
            )
            await ctx.send(view=view)
            return
        ...
        view = CV2View(
            f"{TICK} Chatbot IA activé",
            f"AI chatbot has been enabled in {target_channel.mention}!\nI'll respond to all messages in that channel.",
        )
        await ctx.send(view=view)

    @ai.command(name="deactivate", description="Désactiver le chatbot IA dans le salon")
    async def ai_deactivate(self, ctx: commands.Context):
        """Disable AI chatbot in current server"""
        if not ctx.author.guild_permissions.manage_channels:
            view = CV2View(
                f"{CROSS} Permission refusée",
                "Tu as besoin de la permission `Gérer les salons` pour désactiver le chatbot IA.",
            )
            await ctx.send(view=view)
            return
        ...
        view = CV2View(
            "🔇 Chatbot IA désactivé", "Le chatbot IA a été désactivé sur ce serveur."
        )
        await ctx.send(view=view)

    @ai.command(name="summarize", description="Résumer un long texte")
    ...
    async def ai_summarize(self, ctx: commands.Context, *, text: str):
        ...
        if not text:
            view = CV2View(
                "📝 Résumeur de texte",
                "Fournis un texte à résumer ou réponds à un message.",
            )
            await ctx.send(view=view)
            return
        ...
        try:
            ...
            view = CV2View(
                "📝 Résumé du texte", summary, f"**Original Text:**\n{original_text}"
            )
            await ctx.send(view=view)
        except Exception as e:
            pass

    @ai.command(name="ask", description="Poser une question à l’IA")
    ...
    async def ai_ask(self, ctx: commands.Context, *, question: str):
        ...
        try:
            ...
            view = CV2View(
                "🤖 Réponse de l’IA", answer, f"**Your Question:**\n{question}"
            )
            await ctx.send(view=view)
        except Exception as e:
            pass
    ...
    @ai.command(
        name="fact", description="Obtenir un fait aléatoire ou sur un sujet précis"
    )
    @app_commands.describe(topic="Topic to get a fact about (optional)")
    async def ai_fact(self, ctx: commands.Context, *, topic: str = None):
        """Obtenir un fait aléatoire ou sur un sujet précis"""
        await ctx.defer()
        await self.get_fact(ctx, topic)

    @ai.command(
        name="database-clear",
        description="Efface tes données de conversation IA et ta personnalité",
    )
    async def ai_database_clear(self, ctx: commands.Context):
        """Efface les données de conversation IA et la personnalité de l’utilisateur"""
        await ctx.defer()
        ...
        try:
            ...
            view = CV2View(
                "🗑️ Tes données ont été effacées",
                "Tes données de conversation IA et ta personnalité ont été effacées avec succès. L’IA repartira de zéro avec toi !",
            )
            await ctx.send(view=view, ephemeral=True)

        except Exception as e:
            logger.error(f"Error clearing user AI data: {e}")
            view = CV2View("❌ Erreur", f"Failed to clear your data: {e}")
            await ctx.send(view=view, ephemeral=True)
        ...
        try:
            async with self.bot.db.execute(
                ...
                for role, content, timestamp in reversed(rows):
                    ...
                    if any(
                        keyword in content.lower() for keyword in important_keywords
                    ):
    ...
    async def _save_chat_history(
        self, user_id: int, guild_id: int, message: str, response: str
    ):
    ...
    async def split_and_send(
        self, channel, content: str, reply_to=None, allowed_mentions=None
    ):
    ...
    async def analyze_text(self, ctx, text_content: str):
        ...
        try:
            ...
            analyzed_text = (
                text_content[:512] + "..." if len(text_content) > 512 else text_content
            )
            view = CV2View(
                "📝 Analyse de texte", analysis, f"**Analyzed Text:**\n{analyzed_text}"
            )
            await ctx.send(view=view)
        except Exception as e:
            logger.error(f"Error analyzing text: {e}")
            view = CV2View(
                "📝 Analyse de texte", "Une erreur est survenue pendant l’analyse."
            )
            await ctx.send(view=view)
    ...
    async def generate_trivia_question(self, category: str, used_questions: list):
        ...
        effective_category = (
            category
            if category != "mixed"
            else categories[random.randint(0, len(categories) - 1)]
        )
        category_key = effective_category.replace(" ", "_").lower()

        cached_questions = [
            q
            for q in self.question_cache.get(category_key, [])
        ...
        while attempts < max_attempts:
            ...
            try:
                history = [{"role": "user", "parts": [{"text": prompt}]}]
                text = await self._get_groq_response(
                ...
                if "Question:" in text and "Answer:" in text:
                    ...
                    if question not in used_questions and len(question) > 5:
                        ...
                        if len(self.question_cache[category_key]) > 100:
                            self.question_cache[category_key] = self.question_cache[
                                category_key
                            ][-100:]
                        break
        ...
        if not question_data:
            available_questions = [
                q
                for q in fallback_questions.get(category_key, [])
                if q["question"] not in used_questions
            ]
            if available_questions:
                question_data = available_questions[
                ...
                while len(incorrect_answers) < 2 and incorrect_pool:
                    ...
                    if (
                        ...
                        and incorrect_pool[idx] != question_data["answer"]
                    ):
        ...
        if not question_data:
            logger.error(
                "Failed to generate a unique question after max attempts and fallback"
            )
            return None
    ...
    async def start_trivia_game(self, ctx, category: Optional[str]):
        ...
        if channel_id in self.active_games:
            view = CV2View(
                "🧠 Jeu de quiz", "Un jeu de quiz est déjà actif dans ce salon !"
            )
            await ctx.send(view=view)
            return
        ...
        if not question_data:
            view = CV2View(
                "🧠 Jeu de quiz",
                "Échec de la génération d’une question de quiz. Réessaie plus tard.",
            )
            await ctx.send(view=view)
            return
        ...
        trivia_view = CV2View(
            "🧠 Jeu de quiz",
            f"**Round 1/{self.active_games[channel_id]['max_rounds']}**\nCategory: {category or 'Mixed'}\n{question_data['question']}\n\n*Click a button to answer! First correct answer wins the point.*",
        )
        await ctx.send(view=view)

    async def handle_trivia_answer(
        self, interaction: discord.Interaction, channel_id: int, selected_answer: str
    ):
        ...
        if not game:
            await interaction.followup.send(
                "Aucun jeu de quiz n’est actif dans ce salon !", ephemeral=True
            )
            return

        if game["answered"]:
            await interaction.followup.send(
                "This question has already been answered! Wait for the next round.",
                ephemeral=True,
            )
            return
        ...
        question_data = await self.generate_trivia_question(
            game["category"], recent_questions
        )
        if not question_data:
            del self.active_games[channel_id]
            view = CV2View(
                "🧠 Jeu de quiz",
                "Échec de la génération de la prochaine question. Partie terminée.",
            )
        ...
        if game["round"] > game["max_rounds"]:
            ...
            for uid, score in game["scores"].items():
                ...
                response_message += (
                    f"- {user .display_name if user else 'Unknown'}: {score}\n"
                )
            del self.active_games[channel_id]
            view = CV2View("🧠 Jeu de quiz", response_message)
            await interaction.followup.send(view=view)
        else:
            response_message += f"\n\n**Round {game['round']}/{game['max_rounds']}**\n{game['current_question']}"
            view = TriviaAnswerView(
                self, channel_id, game["current_answer"], game["incorrect_answers"]
            )
            trivia_view = CV2View(
                "🧠 Jeu de quiz",
                response_message
    ...
    async def show_stats(self, ctx):
        ...
        if not stats:
            view = CV2View(
                ...
                "Tu n’as pas encore joué aux quiz ! Commences-en un avec `/ai trivia`.",
            )
            await ctx.send(view=view)
            return
    ...
    async def show_leaderboard(self, ctx):
        ...
        if not top_scores:
            view = CV2View(
                "🏆 Classement du quiz",
                "Aucun score pour le moment ! Joue à un quiz pour commencer.",
            )
            await ctx.send(view=view)
            return
        ...
        view = CV2View("🏆 Classement du quiz", leaderboard)
        await ctx.send(view=view)

    async def enable_roleplay(self, ctx):
        """Activer le mode jeu de rôle dans le salon actuel"""
        channel_id = ctx.channel.id
        user_id = ctx.author.id
        ...
        if channel_id in self.roleplay_channels:
            view = CV2View(
                "🎭 Mode jeu de rôle",
                "Le mode jeu de rôle est déjà activé dans ce salon ! Utilise `/ai roleplay-disable` pour le désactiver.",
            )
            await ctx.send(view=view)
            return
        ...
        view = CV2View(
            "🎭 Mode jeu de rôle",
            "Roleplay mode activated! To start, tell me what kind of character you want me to be.\nFor example: `female teacher` or `male astronaut`.",
        )
        await ctx.send(view=view)

    async def disable_roleplay(self, ctx):
        """Désactiver le mode jeu de rôle dans le salon actuel"""
        channel_id = ctx.channel.id
        if channel_id not in self.roleplay_channels:
            view = CV2View(
                "🎭 Mode jeu de rôle",
                "Le mode jeu de rôle n’est pas activé dans ce salon ! Utilise `/ai roleplay-enable` pour l’activer.",
            )
            await ctx.send(view=view)
            return
        ...
        view = CV2View(
            "🎭 Mode jeu de rôle", "Mode jeu de rôle désactivé dans ce salon."
        )
        await ctx.send(view=view)

    @ai.command(
        name="roleplay-enable",
        description="Activer le mode jeu de rôle dans le salon actuel",
    )
    ...
    @ai.command(
        ...
        description="Désactiver le mode jeu de rôle dans le salon actuel",
    )
