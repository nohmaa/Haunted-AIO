...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_ignore_message(self, ctx, ignore_type: str):
        if ignore_type == "channel":
            await ctx.reply(f"Ce salon est ignoré.", mention_author=False)
        elif ignore_type == "command":
            await ctx.reply(
                f"{ctx.author.mention} Cette commande, ce salon ou toi avez été ignorés ici.",
                delete_after=6,
            )
        elif ignore_type == "user":
            await ctx.reply(f"Tu es ignoré.", mention_author=False)
    ...
    async def command_not_found(self, string: str) -> None:
        ...
        embed = CV2Embed(
            title=f"{BotName} Aide",
            description=f">>> **Oups ! Commande introuvable avec le nom** `{string}`.",
            color=0xFF0000,
        )
    ...
    async def send_bot_help(self, mapping):
        ...
        loading_embed = CV2(f"{LOADINGRED} Chargement du menu d’aide...")
        ...
        filtered = await self.filter_commands(
            self.context.bot.walk_commands(), sort=True
        )
        ...
        embed.set_footer(
            text=f"Requested By {self.context.author} | [Support]({SUPPORT_SERVER})",
        )
    ...
    async def send_command_help(self, command):
        ...
        zyrox = f">>> {command.help}" if command.help else ">>> Non Help Provided..."
        embed = CV2Embed(description=f"""{zyrox}""", color=color)
        alias = " & ".join(command.aliases)

        embed.add_field(
            name="**Alt cmd**",
            value=f"```{alias}```" if command.aliases else "Non Alt cmd",
            inline=False,
        )
        embed.add_field(
            name="**Usage**",
        ...
        embed.set_footer(
            text="<[] = optional | < > = required • Use Prefix Before Commands."
        )
    ...
    def common_command_formatting(self, embed_like, command):
        ...
        if command.description:
            ...
            embed_like.description = command.help or "Aucune aide trouvée..."
...
class Help(Cog, name="help"):
    ...
    def __init__(self, client: zyrox):
        ...
        attributes = {
            ...
            "help": "Affiche l’aide du bot, d’une commande ou d’une catégorie",
        }
