...
class AfkTypeView(LayoutView):
    def __init__(self, author, reason, timeout=60):
        ...
        self.global_btn = Button(label="AFK Global", style=discord.ButtonStyle.primary)
        self.local_btn = Button(label="AFK Local", style=discord.ButtonStyle.success)
        ...
        self.add_item(
            build_container(
                TextDisplay(f"Tu passes AFK pour la raison : **{reason}**"),
                Separator(visible=True),
                TextDisplay("Sélectionne ton type d’AFK avec les boutons ci-dessous."),
    ...
    async def interaction_check(self, interaction: discord.Interaction):
        if interaction.user.id != self.author.id:
            await interaction.response.send_message(
                f"Seul **{self.author.display_name}** peut utiliser ce bouton.",
                ephemeral=True,
            )
            return False
        return True
...
class AfkSuccess(LayoutView):
    def __init__(self, type_val, reason):
        ...
        self.add_item(
            build_container(
                TextDisplay(f"{TICK} **AFK activé**"),
                Separator(visible=True),
                TextDisplay(
                    f"**{MENTION} You are now marked as {type_val.capitalize()} AFK.**\n{SEED} **Reason:** {reason}"
                ),
            )
        )
    ...
    def __init__(self, user_mention, afk_time, reason):
        ...
        self.add_item(
            build_container(
                ...
                Separator(visible=True),
                TextDisplay(
                    f"Went AFK <t:{int(afk_time)}:R> for the following reason:\n**{reason}**"
                ),
            )
        )
...
class afk(commands.Cog):
    ...
    async def clear_afk(self, message):
        async with aiosqlite.connect(DB_PATH) as db:
            cursor = await db.execute(
                "SELECT 1 FROM afk_guild WHERE user_id = ? AND guild_id = ?",
            ...
            cursor = await db.execute(
                "SELECT type, time, mentions FROM afk WHERE user_id = ?",
                (message.author.id,),
            )
            ...
            if afk_type == "global":
                ...
                await db.execute(
                    "DELETE FROM afk_guild WHERE user_id = ?", (message.author.id,)
                )
            else:
                await db.execute(
                    "DELETE FROM afk_guild WHERE user_id = ? AND guild_id = ?",
                    (message.author.id, message.guild.id),
                )
                cursor_check = await db.execute(
                    "SELECT 1 FROM afk_guild WHERE user_id = ?", (message.author.id,)
                )
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            for mentioned in message.mentions:
                ...
                if await cursor.fetchone():
                    cursor_main = await db.execute(
                        "SELECT reason, mentions, time FROM afk WHERE user_id = ?",
                        (mentioned.id,),
                    )
                    ...
                    await message.reply(
                        view=view, delete_after=10, mention_author=False
                    )

                    new_mentions = mentions + 1
                    await db.execute(
                        "UPDATE afk SET mentions = ? WHERE user_id = ?",
                        (new_mentions, mentioned.id),
                    )
                    await db.commit()

                    dm_embed = discord.Embed(
                        description=f"Tu as été mentionné sur **{message.guild.name}** par **{message.author}**",
                        color=self.theme_color,
                    )
                    dm_embed.add_field(name="Mentions totales", value=str(new_mentions))
                    dm_embed.add_field(
                        name="Aller au message",
                        value=f"[Cliquer ici]({message.jump_url})",
                    )
                    dm_embed.set_footer(
                        text=FOOTER_TEXT, icon_url=self.bot.user.avatar.url
                    )
                    try:
    ...
    @commands.hybrid_command(
        name="afk",
        description="Définis ton statut AFK avec une raison (Global ou Local).",
    )
    @blacklist_check()
    @ignore_check()
    @commands.guild_only()
    ...
    async def afk(self, ctx: commands.Context, *, reason: str = "I am AFK"):
        if any(w in reason.lower() for w in ("discord.gg", "gg/")):
            return await ctx.send(
                embed=discord.Embed(
                    description="⚠️ La publicité n’est pas autorisée dans les raisons AFK.",
                    color=self.theme_color,
                ),
                ephemeral=True,
            )
        ...
        if not type_view.value:
            await msg.edit(content="Temps écoulé.", view=None)
            return
