...
class AvatarView(View):
    def __init__(self, user, member, author_id, banner_url):
        ...
        if self.user.avatar.is_animated():
            self.add_item(
                Button(
                    label="GIF",
                    url=self.user.avatar.with_format("gif").url,
        ...
        self.add_item(
            Button(
                label="PNG",
                url=self.user.avatar.with_format("png").url,
                style=discord.ButtonStyle.link,
            )
        )
        self.add_item(
            Button(
                label="JPEG",
                url=self.user.avatar.with_format("jpg").url,
                style=discord.ButtonStyle.link,
            )
        )
        self.add_item(
            Button(
                label="WEBP",
                url=self.user.avatar.with_format("webp").url,
                style=discord.ButtonStyle.link,
            )
        )
    ...
    @discord.ui.button(
        label="Avatar du serveur",
        style=discord.ButtonStyle.success,
        custom_id="server_avatar_button",
    )
    async def server_avatar(self, interaction: discord.Interaction, button: Button):
        if not self.member.guild_avatar:
            await interaction.response.send_message(
                "Cet utilisateur n’a pas d’avatar de serveur différent.", ephemeral=True
            )
        else:
    ...
    @discord.ui.button(
        label="Bannière de l’utilisateur",
        style=discord.ButtonStyle.success,
    ...
    async def banner(self, interaction: discord.Interaction, button: Button):
        if not self.banner_url:
            await interaction.response.send_message(
                "Cet utilisateur n’a pas de bannière.", ephemeral=True
            )
        else:
            embed = interaction.message.embeds[0]
...
class General(commands.Cog):
    ...
    @commands.hybrid_command(
        ...
        name="avatar",
        aliases=["av"],
        help="Obtenir l’avatar utilisateur/avatar de serveur et la bannière d’un utilisateur.",
    )
    @blacklist_check()
    @ignore_check()
    ...
    @commands.hybrid_command(
        name="servericon", help="Obtenir l’icône du serveur", usage="Servericon"
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def servericon(self, ctx: commands.Context):
        ...
        if server.icon is None:
            await ctx.reply(view=CV2("❌ Erreur", "Ce serveur n’a pas d’icône."))
            return
        ...
        dl_btn = Button(
            label="Télécharger l’icône",
            url=str(server.icon.url),
            style=ButtonStyle.link,
        )
        view.add_item(
            build_container(
                TextDisplay(f"**Icône de {server}**"),
    ...
    @commands.hybrid_command(
        name="membercount",
        help="Obtenir le nombre total de membres du serveur",
        usage="membercount",
        aliases=["mc"],
    )
    @blacklist_check()
    ...
    async def membercount(self, ctx: commands.Context):
        ...
        online = len(
            [m for m in ctx.guild.members if m.status == discord.Status.online]
        )
        offline = len(
        ...
        dnd = len(
            [m for m in ctx.guild.members if m.status == discord.Status.do_not_disturb]
        )
        ...
        stats_text = (
            ...
            f"En ligne : {online} | DND : {dnd} | Inactif : {idle} | Hors ligne : {offline}"
        )
        await ctx.send(view=CV2("Statistiques des membres", stats_text))
    ...
    async def poll(self, ctx: commands.Context, *, message):
        author = ctx.author
        msg = await ctx.send(view=CV2(f"**Sondage lancé par {author} !**", message))
        await msg.add_reaction(TICK)
        await msg.add_reaction(CROSS)

    @commands.command(
        name="hack",
        help="pirater le compte discord de quelqu’un",
        usage="Hack <member>",
    )
    @blacklist_check()
    @ignore_check()
    ...
    async def hack(self, ctx: commands.Context, member: discord.Member):
        ...
        lund = await ctx.send(f"Piratage de {member.mention} en cours...")
        ...
        hack_text = (
            ...
            f"**Mot de passe du compte :** {member.name}@{random_pass2}"
        )
        await ctx.send(view=CV2(f"{member.display_name} piraté !", hack_text))
        await lund.delete()
    ...
    async def token(self, ctx: commands.Context, user: discord.Member = None):
        ...
        if user is None:
            user = ctx.author
        await ctx.send(
            view=CV2("🔐 Token", f"{user.mention} — token : `{''.join(token)}`")
        )

    @commands.command(
        name="users", help=f"vérifie le nombre total d’utilisateurs de {BotName}."
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def users(self, ctx: commands.Context):
        ...
        await ctx.send(
            view=CV2(
                f"{BotName} Users",
                f"❯ Total de __**{users}**__ utilisateurs sur **{guilds}** serveurs",
            )
        )
    ...
    async def wizz(self, ctx: commands.Context):
        message6 = await ctx.send(f"`Wizz de {ctx.guild.name}, prendra 22 secondes`")
        message7 = await ctx.send(f"Modification de tous les paramètres du serveur...")
        message5 = await ctx.send(f"Suppression de **{len(ctx.guild.roles)}** rôles...")
        await asyncio.sleep(1)
        message4 = await ctx.send(
            f"Suppression de **{len(ctx.guild.channels)}** salons..."
        )
        await asyncio.sleep(1)
        message3 = await ctx.send(f"Suppression des webhooks...")
        message2 = await ctx.send(f"Suppression des emojis")
        await asyncio.sleep(1)
        message1 = await ctx.send(f"Installation de la vague de bans...")
        await asyncio.sleep(1)
        ...
        await ctx.send(
            view=CV2(
                ...
                f"**{ZWARNING} {ctx.guild.name} wizz avec succès**",
            )
        )

    @commands.hybrid_command(
        name="urban",
        description="Recherche une expression précise sur urbandictionary",
        help="Obtenir la signification d’une expression",
        usage="Urban <phrase>",
    )
    @blacklist_check()
    @ignore_check()
        ...
        async with self.aiohttp.get(
            ...
            try:
                ...
                temp = await ctx.reply(
                    view=CV2(f'Meaning of "{phrase}"', urban_text), mention_author=True
                )
    ...
    @commands.command(
        name="rickroll",
        help="Détecte si l’URL fournie est un rick-roll",
        usage="Rickroll <url>",
    )
    @blacklist_check()
    @ignore_check()
    ...
    async def rickroll(self, ctx: commands.Context, *, url: str):
        if not re.match(self._URL_REGEX, url):
            raise BadArgument("URL invalide")

        phrases = ["rickroll", "rick roll", "rick astley", "never gonna give you up"]
        source = str(
            await (await self.aiohttp.get(url, allow_redirects=True)).content.read()
        ).lower()
        rickRoll = bool(
            (re.findall("|".join(phrases), source, re.MULTILINE | re.IGNORECASE))
        )
        title = "Rick Roll {} in webpage".format(
            "a été trouvé" if rickRoll else "n’a pas été trouvé"
        )
        await ctx.reply(
            view=CV2(
                title, "🎵 Never gonna give you up..." if rickRoll else "✅ Lien sûr !"
            ),
            mention_author=True,
        )

    @commands.command(
        name="hash", help="Hache le texte fourni avec l’algorithme fourni"
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def hash(self, ctx: commands.Context, algorithm: str, *, message):
        ...
        await ctx.reply(
            view=CV2(f'Hashed "{message}"', hash_lines), mention_author=True
        )

    @commands.command(
        name="invite",
        aliases=["invite-bot"],
        description="Obtenir le support et le lien d’invitation du bot !",
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    async def invite(self, ctx: commands.Context):
        invite_text = (
            "```Propulse ton serveur avec des fonctionnalités ultra-rapides et un support 24/7 !```\n"
        ...
        await ctx.send(
            view=CV2(f"{ZYROXCONNECTION} {BotName} Integration Hub!", invite_text)
        )
