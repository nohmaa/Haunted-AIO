...
class Giveaway(commands.Cog):
        ...
        try:
            ...
            if channel is not None:
                try:
                    ...
                    if len(users) < 1:
                        await message.reply(
                            f"Personne n’a gagné le giveaway **{giveaway[5]}**, faute de participants suffisants."
                        )
                        await self.cursor.execute(
                            "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                    ...
                    winner = ", ".join(
                        f"<@!{i}>" for i in random.sample(users, k=winners_count)
                    )
                    ...
                    await message.edit(
                        content=f"{TADAA} **GIVEAWAY ENDED** {TADAA}", view=view
                    )
                    await message.reply(
                        f"{TADAA} Congrats {winner}, you won **{giveaway[5]}!**, Hosted by <@{int(giveaway[3])}>"
                    )
                    await self.cursor.execute(
                        "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                        (message.id, message.guild.id),
    ...
    @commands.hybrid_command(description="Démarre un nouveau giveaway.")
    @blacklist_check()
    @ignore_check()
    ...
    async def gstart(self, ctx, time, winners: int, *, prize: str):
        ...
        await self.cursor.execute(
            "SELECT message_id, channel_id FROM Giveaway WHERE guild_id = ?",
            (ctx.guild.id,),
        )
        ...
        if winners >= 15:
            message = await ctx.send(
                view=CV2("⚠️ Accès refusé", "On ne peut pas dépasser 15 gagnants.")
            )
        ...
        if len(g_list) >= 5:
            message = await ctx.send(
                view=CV2(
                    "⚠️ Accès refusé",
                    "Tu ne peux organiser que 5 giveaways maximum sur ce serveur.",
                )
            )
            await asyncio.sleep(5)
            await message.delete()
        ...
        if converted / 60 >= 50400:
            message = await ctx.send(
                view=CV2("⚠️ Accès refusé", "La durée ne peut pas dépasser 31 jours !")
            )
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if converted == -1:
            message = await ctx.send(view=CV2("❌ Erreur", "Format de durée invalide"))
            await asyncio.sleep(5)
            await message.delete()
            return
        if converted == -2:
            message = await ctx.send(
                view=CV2(
                    "❌ Erreur",
                    "Format de durée invalide. Please provide the time in numbers.",
                )
            )
            await asyncio.sleep(5)
            await message.delete()
    ...
    async def GiveawayMessageDelete(self, message):
        ...
        if re is not None:
            if message.id == int(re[0]):
                ...
                print(
                    f"Giveaway message deleted in {message.guild.name} - {message.guild.id}"
                )
                await self.connection.commit()

    @commands.hybrid_command(
        name="gend",
        description="Termine un giveaway avant sa fin.",
        help="Termine un giveaway avant sa fin.",
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def gend(self, ctx, message_id=None):
        if message_id:
            try:
                ...
                message = await ctx.send(
                    view=CV2("⚠️ Accès refusé", "ID de message invalide fourni.")
                )
                await asyncio.sleep(5)
                await message.delete()
                return
        ...
        if message_id is not None:
            current_time = datetime.datetime.now().timestamp()
            await self.cursor.execute(
                "SELECT ends_at, guild_id, message_id, host_id, winners, prize, channel_id FROM Giveaway WHERE message_id = ?",
                (int(message_id),),
            )
            re = await self.cursor.fetchone()
            ...
            if re is None:
                message = await ctx.send(
                    view=CV2("❌ Erreur", "Le giveaway est introuvable.")
                )
                await asyncio.sleep(5)
                await message.delete()
                return
            ...
            if len(users) < 1:
                await ctx.send(
                    f"{TICK} Giveaway terminé avec succès dans <#{int(re[6])}>"
                )
                await message.reply(
                    f"Personne n’a gagné le giveaway **{re[5]}**, faute de participants suffisants."
                )
                await self.cursor.execute(
                    "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                    (message.id, message.guild.id),
            ...
            if int(ctx.channel.id) != int(re[6]):
                await ctx.send(
                    f"{TADAA} Giveaway terminé avec succès dans <#{int(re[6])}>"
                )

            await message.reply(
                f" Congrats {winner}, you won **{re[5]}!**, Hosted by <@{int(re[3])}>"
            )
            await self.cursor.execute(
                "DELETE FROM Giveaway WHERE message_id = ? AND guild_id = ?",
                (message.id, message.guild.id),
            ...
            if re is None:
                return await ctx.send(f"Le giveaway est introuvable.")
            ...
            await ctx.send(
                "Réponds au message du giveaway ou fournis l’ID du giveaway."
            )
        await self.connection.commit()

    @commands.hybrid_command(
        description="Relance un giveaway en répondant au message du giveaway.",
        help="Relance un giveaway en répondant au message du giveaway.",
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def greroll(self, ctx, message_id: typing.Optional[int] = None):
        if not ctx.message.reference:
            message = await ctx.reply(
                "Réponds à cette commande avec le message de fin du giveaway pour relancer."
            )
            await asyncio.sleep(5)
            await message.delete()
            return
        ...
        if ctx.message.reference.resolved.author.id != self.bot.user.id:
            msg = await ctx.send(
                view=CV2("⚠️ Accès refusé", "Le giveaway est introuvable.")
            )
            await asyncio.sleep(5)
            await msg.delete()
            return
        ...
        await self.cursor.execute(
            f"SELECT message_id FROM Giveaway WHERE message_id = ?", (message.id,)
        )
        re = await self.cursor.fetchone()
        ...
        if re is not None:
            msg = await ctx.send(
                view=CV2(
                    "⚠️ Accès refusé",
                    "Le giveaway est en cours. Utilise plutôt la commande `gend` pour le terminer.",
                )
            )
            await asyncio.sleep(5)
            await msg.delete()
        ...
        await message.reply(
            f" Le nouveau gagnant est "
            + ", ".join(f"<@{i}>" for i in winners)
            + ". Congratulations!"
        )
        await self.connection.commit()
    ...
    @commands.hybrid_command(
        name="glist", description="Liste tous les giveaways en cours."
    )
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    ...
    async def glist(self, ctx):
        await self.cursor.execute(
            "SELECT prize, ends_at, winners, message_id FROM Giveaway WHERE guild_id = ?",
            (ctx.guild.id,),
        )
        giveaways = await self.cursor.fetchall()

        if not giveaways:
            await ctx.send(view=CV2("Giveaways en cours", "Aucun giveaway en cours."))
            return

        desc = ""
        for giveaway in giveaways:
            prize, ends_at, winners, message_id = giveaway
            desc += f"**{prize}**\nEnds: <t:{int(ends_at)}:R> (<t:{int(ends_at)}:f>)\nWinners: {winners}\n[Aller au message](https://discord.com/channels/{ctx.guild.id}/{ctx.channel.id}/{message_id})\n\n"

        await ctx.send(view=CV2("Giveaways en cours", desc))
