...
class AntiLink(commands.Cog):
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(title="Log Automod : Anti-Liens", color=0xFF0000)
    ...
    async def on_message(self, message):
        ...
        if not await self.is_automod_enabled(
            guild_id
        ) or not await self.is_anti_link_enabled(guild_id):
            return
        ...
        if self.link_pattern.search(message.content):
            ...
            try:
                if punishment == "Mute":
                    ...
                    action_taken = "Exclu temporairement pendant 7 minutes"
                ...
                simple_embed = discord.Embed(title="Automod Anti-Liens", color=0xFF0000)
                simple_embed.description = f"{TICK} | {user.mention} a été **{action_taken}** avec succès pour **avoir posté un lien.**"
