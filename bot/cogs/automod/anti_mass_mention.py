...
class AntiMassMention(commands.Cog):
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(
                    title="Log Automod : Anti Mentions de Masse", color=0xFF0000
                )
    ...
    async def on_message(self, message):
        ...
        if not await self.is_automod_enabled(
            guild_id
        ) or not await self.is_anti_mass_mention_enabled(guild_id):
            return
        ...
        if mention_count >= self.mass_mention_threshold:
            ...
            try:
                ...
                simple_embed = discord.Embed(
                    title="Automod Anti Mentions de Masse", color=0xFF0000
                )
                simple_embed.description = f"{TICK} | {user.mention} a été **{action_taken}** avec succès pour **mentions de masse.**"
