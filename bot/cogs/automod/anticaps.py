...
class AntiCaps(commands.Cog):
    ...
    async def is_anti_caps_enabled(self, guild_id):
        async with aiosqlite.connect("db/automod.db") as db:
            cursor = await db.execute(
                "SELECT punishment FROM automod_punishments WHERE guild_id = ? AND event = 'Anti caps'",
                (guild_id,),
            )
            result = await cursor.fetchone()
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(
                    title="Log Automod : Anti-Majuscules", color=0xFF0000
                )
    ...
    async def on_message(self, message):
        ...
        if not await self.is_automod_enabled(
            guild_id
        ) or not await self.is_anti_caps_enabled(guild_id):
            return
            ...
            if caps_percentage > self.caps_threshold:
                ...
                try:
                    if punishment == "Mute":
                        ...
                        await user.edit(
                            timed_out_until=timeout_duration,
                            reason="Majuscules excessives",
                        )
                        action_taken = "Exclu temporairement pendant 1 minute"
                    elif punishment == "Kick":
                        await user.kick(reason="Majuscules excessives")
                        action_taken = "Expulsé"
                    elif punishment == "Ban":
                        await user.ban(reason="Majuscules excessives")
                        action_taken = "Banni"
                    await message.delete()

                    simple_embed = discord.Embed(
                        title="Automod Anti-Majuscules", color=0xFF0000
                    )
                    simple_embed.description = f"{TICK} | {user.mention} a été **{action_taken}** avec succès pour **majuscules excessives.**"
