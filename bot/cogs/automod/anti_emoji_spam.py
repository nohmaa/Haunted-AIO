...
class AntiEmojiSpam(commands.Cog):
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(
                    title="Log Automod : Anti Spam d’Emojis", color=0xFF0000
                )
    ...
    async def on_message(self, message):
        ...
        if not await self.is_automod_enabled(
            guild_id
        ) or not await self.is_anti_emoji_spam_enabled(guild_id):
            return
        ...
        if emoji_count > self.emoji_threshold:
            ...
            action_taken = None
            reason = f"Spam d’emojis ({emoji_count} emojis)"
            ...
            try:
                ...
                simple_embed = discord.Embed(
                    title="Automod Anti Spam d’Emojis", color=0xFF0000
                )
                simple_embed.description = f"{TICK} | {user.mention} a été **{action_taken}** avec succès pour **spam d’emojis.**"
