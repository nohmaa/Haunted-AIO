...
class AntiInvite(commands.Cog):
    ...
    async def log_action(self, guild, user, channel, action, reason):
        ...
        if log_channel_id and log_channel_id[0]:
            ...
            if log_channel:
                embed = discord.Embed(
                    title="Log Automod : Anti-Invitations", color=0xFF0000
                )
    ...
    async def on_message(self, message):
        ...
        if not await self.is_automod_enabled(
            guild_id
        ) or not await self.is_anti_invites_enabled(guild_id):
            return
        ...
        if self.invite_pattern.search(message.content):
            ...
            try:
                ...
                action_taken = None
                reason = "Lien d’invitation posté"

                try:
                    if punishment == "Mute":
                        timeout_duration = discord.utils.utcnow() + timedelta(
                            minutes=12
                        )
                        await user.edit(
                        ...
                        await user.kick(reason="Lien d’invitation posté")
                        action_taken = "Expulsé"
                    elif punishment == "Ban":
                        await user.ban(reason="Lien d’invitation posté")
                        action_taken = "Banni"

                    await message.delete()

                    simple_embed = discord.Embed(
                        title="Automod Anti-Invitations", color=0xFF0000
                    )
                    simple_embed.description = f"{TICK} | {user.mention} a été **{action_taken}** avec succès pour **avoir posté un lien d’invitation.**"
