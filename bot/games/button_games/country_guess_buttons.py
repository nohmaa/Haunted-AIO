...
class CountryInput(discord.ui.Modal, title="Entre ta réponse !"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if guess == game.country:
            game.update_guesslog("+ PARTIE TERMINÉE, gagné ! +")
            await interaction.response.send_message(
                f"Bonne réponse ! Le pays était `{game.country.title()}`"
            )
            ...
            if not game.guesses:
                self.view.disable_all()
                game.update_guesslog("- PARTIE TERMINÉE, perdu -")

                await interaction.message.edit(embed=game.embed, view=self.view)
                await interaction.response.send_message(
                    f"Partie terminée ! Tu as perdu, le pays était `{game.country.title()}`"
                )
                return self.view.stop()
            else:
                acc = game.get_accuracy(guess)
                game.update_guesslog(
                    f"- [{guess}] était incorrect ! Mais tu es à ({acc}%) du but !\n"
                    f"+ Il te reste {game.guesses} essais.\n"
                )
...
class CountryView(BaseView):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.user:
            await interaction.response.send_message(
                f"Ce n’est pas ta partie !", ephemeral=True
            )
            return False
        else:
            return True

    @discord.ui.button(label="Deviner !", style=discord.ButtonStyle.blurple)
    ...
    @discord.ui.button(label="indice", style=discord.ButtonStyle.green)
    async def hint_button(
        ...
        self.game.hints -= 1
        await interaction.response.send_message(
            f"Voici ton indice : `{hint}`", ephemeral=True
        )
    ...
    async def cancel_button(self, interaction: discord.Interaction, _) -> None:
        ...
        self.game.update_guesslog("- PARTIE TERMINÉE, ANNULÉE -")

        await interaction.response.send_message(
            f"Partie terminée ! Le pays était `{self.game.country.title()}`"
        )
...
class BetaCountryGuesser(CountryGuesser):
    ...
    def update_guesslog(self, entry: str) -> None:
        ...
        self.embed.set_field_at(
            1, name="Journal des essais", value=f"```diff\n{self.guesslog}\n```"
        )
    ...
    async def start(
        ...
        *,
        ...
        self.embed.add_field(
            name="Journal des essais", value="```diff\n\u200b\n```", inline=False
        )
