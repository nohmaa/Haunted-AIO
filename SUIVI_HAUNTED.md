...
- **Bug réel corrigé** : `send_group_help` appelait `FieldPagePaginator(...).get_pages()` (méthode inexistante) puis `Paginator(ctx, embeds)` (mauvais arguments) → `!help <groupe>` plantait en `AttributeError`. Remplacé par `Paginator(source=FieldPagePaginator(...), ctx=...)` (comme `send_cog_help`). Vérifié : un embed valide part maintenant.
- **Filet de sécurité global (`core/Context.py`)** : `send`/`reply` passent par `_deliver()` ; si Discord répond **50006/50035**, le contenu est reconstitué en **texte brut** (embeds via `to_dict()`, vues CV2 via `_title/_description/_fields/_footer`, composants via l'arbre `.children`) et renvoyé **sans** `embed/embeds/view`, le reste des kwargs (fichiers, `delete_after`…) étant préservé — avec repli final sur `channel.send` si la référence du reply est morte. Les autres codes (ex. 50013 Forbidden) remontent normalement. Un avertissement console unique explique la cause. → toutes les commandes à embed/composants continuent d'afficher leur contenu au lieu d'échouer.
- **Erreurs plus lisibles (`cogs/events/Errors.py`)** : (1) **traceback complet** imprimé pour `CommandInvokeError` (avant : seulement le message) ; (2) `InvalidNodeException`/`NodeException`/`PoolException`/`LavalinkException` → message FR clair pour l'utilisateur (« aucun serveur musical connecté, réessai automatique ») au lieu d'une exception brute.
- **Musique (`cogs/commands/music.py`)** : `connect_nodes` (1) normalise l'URI Lavalink (`lava.exemple.fr`, `host:port`, `https://host`, `wss://…` → fini l'URI invalide `https://https://…`), (2) **retente toutes les 30 s** jusqu'à enregistrement du noeud (avant : échec silencieux → `InvalidNodeException` sur toutes les commandes musique), (3) journalise URI/statut ou la cause d'échec. Attention : `wavelink.Pool.connect` **n'élève pas** d'exception, il journalise (les lignes wavelink apparaissent maintenant dans la console). Le noeud par défaut `lava-v4.ajieblogs.eu.org` a été testé vivant (`/version` → 200).
- **À vérifier côté serveur** : permission **Intégrer des liens** (+ « Envoyer des messages ») du bot dans les salons concernés ; `LAVALINK_HOST` / `LAVALINK_PASSWORD` du `.env` de prod (hôte, port, mot de passe réels) ; dépendances à jour sur le disque Pterodactyl (`discord.py==2.7.1` requis pour les composants v2).
- **Vérifications** : `compileall` OK ; harnais venv (dpy 2.7.1) — repli texte sur 50006 (embed → texte, CV2 → texte), 50013 non avalé, `send_group_help` OK, `import cogs` OK (162 symboles), `import haunted` OK ; `_lavalink_uri` validée sur 8 combinaisons de `.env` ; `black --check` propre sur les blocs ajoutés.
...
- **Fixes** : (1) `export const dynamic = "force-dynamic"` ajouté aux 23 pages concernées (script) — attention, 8 d'entre elles importaient aussi `dynamic` de `next/dynamic` : import renommé `nextDynamic` pour éviter la collision ; (2) `lib/api.ts` : les erreurs avec digest `NEXT_*` (DYNAMIC_SERVER_USAGE, NEXT_REDIRECT…) ne sont **jamais retentées** — elles sont du contrôle de rendu Next, pas des échecs réseau.
- **Vérification** : `npm run build` OK, **zéro** warning `Dynamic server usage`, toutes les routes dashboard en `ƒ` (dynamique) au lieu de statiques avec état figé. Redeploy Vercel requis.

## 2026-09-23 — « fetch failed » sur la page Serveurs : retry + tunnel en HTTP/2
- **Reproduction navigateur** : `/dashboard/guilds` affiche « Erreur de connexion — fetch failed » alors que la console bot voit `/guilds/` en 200 → le fetch qui échoue part de la **fonction serveur Next.js sur Vercel** et n'atteint jamais le bot. Salve curl 15/15 OK depuis l'Europe : le tunnel flotte par intermittence (périodes 530 observées plus tôt).
- **Fix 1 (dashboard)** : `lib/api.ts` — retry avec backoff (3 essais, 400 ms × n) sur les **échecs réseau uniquement** (tunnel down, timeout 15 s via `AbortSignal.timeout`) ; les erreurs HTTP (401/404/500) restent fatales sans retry (déterministes). GET et mutations idempotentes (configs) → retry sûr.
- **Fix 2 (bot, cause racine probable)** : `utils/tunnel.py` force `--protocol http2` (HTTP/2 sur TCP) au lieu de QUIC (UDP). Les hébergeurs type Pterodactyl brident/filtrent souvent l'UDP sortant → tunnel qui flotte. `--url` conservé (no-op en tunnel nommé, déjà comme ça).
- **Déploiement** : (a) côté Pterodactyl → `Restart` (récupère le fix tunnel via AUTO_UPDATE) ; (b) côté Vercel → **Redeploy** du dashboard (récupère le retry). Vérif post-déploiement : ligne `[cloudflared]` qui mentionne `protocol=http2`, et page Serveurs qui liste les serveurs.
- Si des 530 subsistent malgré http2 : vérifier que l'hébergeur ne coupe pas les connexions TCP longues, envisager de lancer cloudflared hors du conteneur.

## 2026-09-23 — Confirmations prod + logs API en double (corrigé)
- **Prod OK après Restart** (log utilisateur) : `Loaded & Online!` (2 guilds), tunnel reconnecté (cdg17), 264 commandes + 89 slash synchronisées, et surtout **toutes les requêtes dashboard → API en 200** (`/admin/config`, `/bot/info`, `/guilds/`, `/admin/stats`, 2-5 ms). Le problème « dashboard ne récupère plus les données » est résolu (cause : tunnel down, cf. entrée précédente).
- **Cosmétique corrigé** : chaque ligne de log API apparaissait **deux fois** dans la console (JSON brut + `[HH:MM:SS] -> JSON`). Cause : des cogs (`autorole.py`, `booster.py`, …) appellent `logging.basicConfig(...)` à l'import, ce qui ajoute un handler au logger racine ; le logger `api_request_logs` s'y propageait. Fix : `logger.propagate = False` dans `api/server.py`. Test en venv avec un `basicConfig` de cog chargé avant l'API : ligne émise exactement 1 fois.

## 2026-09-23 — Dashboard « FETCH FAILED » : tunnel Cloudflare down + durcissements
- **Cause racine (infra, pas le code)** : le dashboard Vercel affichait `FETCH FAILED` car le tunnel `api.haunted-mind.site` renvoyait **HTTP 530** (Cloudflare 1033 = tunnel déconnecté) pendant ~25 min (13h41→14h06). Mesures : preflight CORS 530 vu depuis le navigateur, 6×530 en sondage, retour à la normale ensuite. L'API/auth/CORS/clé fonctionnaient (vérifié par curl : 401 propre avec mauvaise clé, preflight apex 200 avec `access-control-allow-origin`). Le tunnel tourne dans le processus du bot → 530 prolongé = processus bot down ou réseau coupé. Le code du tunnel se reconnecte seul toutes les 5 s (`utils/tunnel.py`). **Si ça se reproduit : regarder la console Pterodactyl** (processus arrêté / crash-loop / log `[cloudflared]`).
- **Amélioration monitoring** : `/health` et `/` sont désormais **publics** (l'auth Bearer était globale sur l'app, y compris le health-check — trompeur en diagnostic). Toutes les routes `/api/v1/*` exigent toujours la clé (`dependencies` déplacées sur les 4 routeurs). Vérifié : app créée OK, routes publiques = `/` + `/health` uniquement.
- **Bug latent corrigé (cycle d'imports utils↔core)** : `utils/Tools.py` importait `from core import Context` sans jamais l'utiliser → ImportError `cannot import name 'getConfig' from 'utils'` si `api.server` était importé avant `core` (fragilité d'ordre au démarrage). Import mort supprimé ; les deux chemins validés en venv : `from api.server import create_app` seul OK, `import haunted` complet OK.
- Enseignement test : un harnais hors-ligne qui charge les cogs sans connexion Discord se bloque sur leurs `wait_until_ready()` — ce n'est PAS un bug du bot ; ne pas interpréter `Failed to load extension` hors contexte.
- À faire côté serveur : `Restart` pour récupérer les correctifs (`AUTO_UPDATE=1`), puis surveiller une éventuelle rechute du tunnel via la console.

## 2026-09-23 — Fix crash démarrage : classe cog renommée par la traduction + .env.example
- **Crash prod** : `ImportError: cannot import name 'Birthdays' from 'cogs.commands.Birthday'`. Un agent de traduction avait renommé la **classe** du cog `Birthdays` → `Anniversaires` (identifiant backend — violation de la règle « backend en anglais »), alors que `cogs/__init__.py` importe toujours `Birthdays`. Le renommage était présent dès le commit de consolidation (le `compileall` ne détecte pas ce type d'erreur : syntaxe OK, résolution de noms KO).
- **Fix** : classe renommée `Anniversaires` → `Birthdays` (+ `client.add_cog(Birthdays(client))` dans le setup du cog) ; les textes UI restent traduits (« Anniversaires du jour », etc.). `cogs/__init__.py` inchangé (déjà cohérent).
- **Audit anti-régression** : comparaison AST de toutes les classes cog NOW vs commit pré-traduction `88924d5` → un seul renommage. Contrôle croisé imports ↔ `bot.add_cog` de `cogs/__init__.py` → tout cohérent (les autres alertes étaient des faux positifs de star-imports).
- **Test d'import réel en venv vierge** : `import cogs` OK (126 symboles) et chargement complet de `haunted.py` OK — c'est exactement la ligne qui crashait en prod. **Nouveau standard de vérification** : compileall + audit AST des renommages + import réel.
- **Warning dotenv** (`could not parse statement starting at line 53`) : non bloquant (python-dotenv saute la ligne). Cause probable côté serveur : `.env` copié depuis l'ancien template qui contenait une ligne `[TEMPLATE]` non parsable (retirée de `bot/.env.example`, vérifié : 19 clés parsées proprement). **À faire côté serveur : vérifier la ligne 53 du `.env` de prod et supprimer toute ligne `[...]` ou mal quotée.**
- Reste à surveiller : toute traduction future ne doit JAMAIS toucher noms de classes/fonctions/variables/comparaisons — UI uniquement.
